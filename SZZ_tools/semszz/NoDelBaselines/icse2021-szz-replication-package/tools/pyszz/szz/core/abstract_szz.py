...
class AbstractSZZ(ABC):
    ...
    def __init__(self, repo_full_name: str, repo_url: str, repos_dir: str = None, use_temp_dir: bool = False):
        """
    ...
    def get_neighbour_lines(self,fix_cid,lines_added,file_path,N=1):
        added_set = set(lines_added)
        neighbour_lines = set()
        source_file_content = self.repository.git.show(f"{fix_cid}:{file_path}")
        max_lineno = len(source_file_content.split('\n'))
        for added_line in lines_added:
            cnt = 0
            for i in range(added_line,0,-1):
                if i not in added_set:
                    neighbour_lines.add(i)
                    cnt = cnt + 1
                    if cnt > N:
                        break
            cnt = 0
            for i in range(added_line,max_lineno):
                if i not in added_set:
                    neighbour_lines.add(i)
                    cnt = cnt + 1
                    if cnt > N:
                        break
        return list(neighbour_lines)
    ...
    def get_impacted_files(self, fix_commit_hash: str,
        ...
        for mod in fix_commit.modifications:
            ...
            if only_deleted_lines:
                ...
                mod_lines = self.get_neighbour_lines(fix_commit_hash,lines_added,file_path)
