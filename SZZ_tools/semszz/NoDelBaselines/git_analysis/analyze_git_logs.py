...
def traverse_affected_versions(git_log_dict, fixing_commit, inducing_commit):
    q = [fixing_commit]
    ...
    while len(q) > 0:
        ...
        gl = git_log_dict[next_commit]
        has_visited_commits.add(next_commit)

        # print(gl.commit_id, gl.tag)
        if gl.tag is not None:
            commits_with_tag.add(gl)

        if next_commit == inducing_commit:
            # print('stop', inducing_commit)
            break
