...
class VSZZ(AbstractSZZ):
    ...
    * ignore_revs_file_path
    ...
    def find_bic(self, fix_commit_hash: str, impacted_files: List['ImpactedFile'], **kwargs) -> Set[Commit]:
        ...
        bug_introd_commits = set()  # 改为集合类型，存储 Commit 对象
        for imp_file in impacted_files:
            # print('impacted file', imp_file.file_path)
            try:
                blame_data = self._blame(
                ...
                for entry in blame_data:
                    ...
                    # 将所有历史提交添加到结果集合中
                    for commit_hash, _, _ in previous_commits:
                        # 通过哈希获取 Commit 对象（假设 self.repository 是 git.Repo 实例）
                        commit = self.repository.commit(commit_hash)
                        bug_introd_commits.add(commit)

            except:
                print(traceback.format_exc())

        return bug_introd_commits  # 返回 Commit 对象集合
    ...
    #         mapping_db = json.load(open(mapping_db_file))
    #         if commit_id not in mapping_db:
    #             subprocess.check_output(mapping_cmd.format(project=self.repo_full_name, commit_id=commit_id,
    #                                                        output=os.path.join(ast_map_temp, "tmp.json"),
    #                                                        file_path=file_path), cwd=self.ast_map_path,
    ...
    #         subprocess.check_output(mapping_cmd.format(project=self.repo_full_name, commit_id=commit_id,
    #                                                    output=os.path.join(ast_map_temp, "tmp.json"),
    #                                                    file_path=file_path), cwd=self.ast_map_path, shell=True).decode(
    #             'utf-8', errors='ignore')
    #
    #         mapping_results = json.load(open(os.path.join(ast_map_temp, "tmp.json")))
    #         mapping_db = {}
